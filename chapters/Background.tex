...
%Pieces of the IoT are already present in everyday's life, such as sensor networks in smart cities, personal smart phones, RFID tags in many retail stores, and so on.
%However, the seamless integration of this pieces still have several issues, from the development and deployment of services, to the usability.
...
%Usually it communicates with the user through a smart phone, and in some cases they embed a user interface (UI), \textit{i.e.} a display in the case of smart watches.
...
\section{Towards an infrastructure for the IoT}
\label{sec:IoTInfra}
...
In figure \ref{fig:SmartCity} we can observe the diversity of participants and their very particular nature, which comes from small embedded environmental sensors to connected cars and smart grids.
...
As residential and commercial buildings are the most energy consuming components \cite{perez2008review} of a city, solutions to this important problem must be found.
...
%This is useful to have a current or previous vision of the building's behaviour, since historical data can be analysed to generate visualizations at any time.
...
This scenario is only possible if an infrastructure composed of all the features described above is deployed in current buildings, as well as fitted by default in new ones to be IoT ready.
...
%Devices like Programmable Logic Controllers\todo{ \cite{}} and supervision software\todo{ \cite{}} are needed to enable local data processing, which can lead to a better understanding of the building's behaviour 
...
%On the other hand, maintenance-free energy harvesting devices presented in the previous section are able to send wireless messages using proprietary protocols, which ease their deployment

This communication approaches can be reused \textit{as is}, in an IoT infrastructure, since the physical medium used to transmit messages is the same.
...
Solutions that come from the Information and Communication Technologies (ICT) field will be studied throughout this thesis, in order to find a new way to develop, deploy and maintain this IoT infrastructure.
...
%Thanks to this, the upper layers can access, in a \textit{"web fashion"} (\textit{i.e.} using OPC, oBIX, etc.) \cite{neugschwandtner2007knxtoobix}, to all this data in order to perform cloud-based manipulations.
...
One of the main reasons for this slower evolution, is the cost of MCUs.
...
Figure \ref{fig:CoAPRequest} shows a typical CoAP request, on which only 13 bytes are needed to format this message, leaving around 90 bytes for payload in a single 802.15.4 frame.
...
Figure \ref{fig:IPLayers} illustrates a layered representation of both stacks, the classical Internet and the IoT.
...
%Other protocol stacks provide similar behaviour than the mentioned above.
...
%Logging data locally for further use can be very useful, but storing configurations and data about network behaviour is even more.
...
Figure \ref{fig:RIOTIPStack} show the modules provided by this OS, as well as the embedded IP stack for networking.
...
%Human activities dedicated to this efficient energy use (\textit{i.e.} remember to turn off lights or HVAC equipment), can be in charge of an IoT infrastructure in the background, bringing comfort and time saving that can be used for familiar activities, sports or hobbies, for instance.
