...
Seen as an IoT infrastructure, in which all devices are able to implement IP stacks, this environment will be able to take advantage of this relationships, using the standard IoT web APIs.
...
Services are now provided in a more direct way, it means that the host of such services are the devices themselves.
Thus, the collaboration between objects is achieved when a device can talk directly to another using Internet, being such devices part of very different sectors of a building or Smart City.
...
Comparing the behavior of both IoT and classical Internet nodes, we found that they have very few differences, being one of the most important the computational resources available and the network architecture.
However, the resemblances such as the quantity of nodes, the application layer and the distributed nature, were enough to explore the already proposed solutions for classical distributed systems.
Previous works using approaches typical of Model Driven Engineering (MDE), have obtained good results adding self-adaptation and reconfiguration mechanisms to this systems.
...
The main goal of the conducted research was to deal with the main architectural (both at the hardware and system level) differences mentioned above, proposing new methods to work with scarce resources and to reduce the inherent energy consumption of the extra network traffic.
