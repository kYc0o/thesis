...
Descriptions of these platforms are based on how the modules or components are described and packaged, how does the deployment descriptor of those is created and in which ways these platforms addressed previously introduced issues.
...
\begin{itemize}
	...
	The execution platform for distributed CORBA based software is called Corba Component Model (CCM)\cite{}, in which a component is defined by an interface and one or more implementations of that interface.
	...
	\item \textbf{EJB.}
	For the construction of enterprise Java applications, an API was specified by Sun Microsystems\cite{} which describes a managed, server-side component architecture called Enterprise JavaBeans (EJB).
...
Thus far, the proposed deployment concept was explained, which can be represented in figure X \todo{make figure of the deployment cycle}, including the creation, distribution and maintenance of a given application.
...
The trade-offs presented in this article for these different approaches are shown in figure \todo{figure 3.10}.
...
\begin{itemize}
	...
	Remote request for files using tools such scp over ssh are often used to copy files, described in configuration files.
	...
	Deployers such as SmartFrog\cite{goldsack2003smartfrog} and \cite{wang2006language} are examples of the uyility of this approach.
	...
	It is also defined an abstraction layer for managing the configurations of deployed software. 
	...
	Custom automation policies seems to be difficult for associating between them, even if the language facilitates specifying the deployment.
...
\end{itemize}
\todo{make a table showing the main advantages and drawbacks of these methods}
...
In conclusion, even if deployment mechanisms for IoT devices exist and can be improved to reach a good reliability level and ease to use, manual installation or updates of features should not be worth considering.
Figure \todo{figure 3.10} already shows that in classical distributed systems manual deployment is not suitable for large-scale systems.
